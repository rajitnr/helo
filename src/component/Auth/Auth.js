import React, { Component } from "react";
import { Link, Redirect } from "react-router-dom";
import axios from "axios";
import { connect } from "react-redux";

import { BASE_URL } from "../../constants";
import { updateUsername, updateProfilePic } from "../../ducks/reducer";

class Auth extends Component {
  constructor() {
    super();
    this.state = {
      username: "",
      password: "",
      profile_pic: "",
      postSuccessful: false,
      loginSuccessful: true
    };
  }
  handleChangeEvent = (key, event) => {
    this.setState({ [key]: event.target.value });
  };
  handleRegister = () => {
    console.log("In for handle submit`");
    let { username, password, profile_pic } = this.state;
    profile_pic = "https://robohash.org/" + { username } + ".png/?bgset=bg2";
    axios
      .post(`${BASE_URL}/user`, { username, password, profile_pic })
      .then(response => {
        const { username } = response.data[0];
        this.setState({ username: username, postSuccessful: true });
        this.props.updateUsername(username);
        this.props.updateProfilePic(profile_pic);
      })
      .catch(() => this.setState({ postSuccessful: false }));
  };
  handleLogin = () => {
    let { username, password, profile_pic } = this.state;
    axios
      .post(`${BASE_URL}/authenticate`, { username, password })
      .then(response => {
        const { username, profile_pic } = response.data[0];
        this.setState({
          loginSuccessful: true,
          postSuccessful: true
        });
        this.props.updateUsername(username);
        this.props.updateProfilePic(profile_pic);
      })
      .catch(() => this.setState({ loginSuccessful: false }));
  };
  componentDidMount = () => {
    this.setState({ loginSuccessful: true, postSuccessful: false });
  };
  render() {
    return (
      <div class="auth-page">
        <div className="auth-container">
          {this.state.postSuccessful && <Redirect to="/dashboard" />}
          <img
            src="data:image/png;base64,"
            alt=""
            className="logo"
          />
          <span className="auth-title">Helo</span>
          <div className="auth-input">
            <label htmlFor="username">Username: </label>
            <input
              type="text"
              name="username"
              id="username"
              onChange={event => this.handleChangeEvent("username", event)}
            />
          </div>
          <div className="auth-input">
            <label htmlFor="password">Password: </label>
            <input
              type="password"
              name="password"
              id="password"
              onChange={event => this.handleChangeEvent("password", event)}
            />
          </div>
          <div className="auth-buttons">
            <button className="btn btn-clickable" onClick={this.handleLogin}>
              Login
            </button>
            <button className="btn btn-clickable" onClick={this.handleRegister}>
              Register
            </button>
          </div>
          <span className="auth-msg">
            {!this.state.loginSuccessful && "Invalid Username Or Password"}
          </span>
        </div>
      </div>
    );
  }
}

// const mapStateToProps = state => {
//   return {
//     username: state.username,
//     profile_pic: state.profile_pic
//   };
// };

export default connect(
  null,
  { updateUsername, updateProfilePic }
)(Auth);
